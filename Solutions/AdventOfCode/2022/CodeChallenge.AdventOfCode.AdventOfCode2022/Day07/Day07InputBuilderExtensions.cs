...
internal static class Day07InputBuilderExtensions
{
    public static IInputProvider<AdventOfCodeChallengeSelection, Directory> BuildDay07InputProvider(
                ...
                foreach (var chunk in lineChunks)
                {
                    ...
                    if (!commandLine.StartsWith('$'))
                    {
                        throw new FormatException(
                            $"Received a command chunk that did not start with a command: '{commandLine}'");
                    }
