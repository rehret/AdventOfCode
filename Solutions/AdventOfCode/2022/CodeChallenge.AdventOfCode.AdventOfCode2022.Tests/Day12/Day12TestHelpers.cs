...
internal static class Day12TestHelpers
{
    public static HeightmapWithStartAndEnd BuildSampleInput(Day12InputProviderBuilderExtensions.DijkstraType type)
    {
    ...
    private static IEnumerable<Coordinate>GetNeighbors(IEnumerable<Coordinate> coordinates, Coordinate coordinate, Day12InputProviderBuilderExtensions.DijkstraType type)
    {
        return coordinates
            .Where(x =>
                ...
                || (x.X == coordinate.X && x.Y == coordinate.Y + 1))
            .Where(x => type == Day12InputProviderBuilderExtensions.DijkstraType.StartIsSource
                    ? x.Height <= coordinate.Height + 1
                    : x.Height >= coordinate.Height - 1);
    }
}